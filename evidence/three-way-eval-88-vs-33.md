...
**Date:** 2026-05-20
**Source:** ~/git/redhat-expert/entries/2026/05/20/three-way-eval-comparison-expert-service-vs-agents-python-across-all-rubrics.md

## Overview

A three-way eval comparison on the same 50-question Red Hat domain sample (seed=42) completes the evidence base for EEM's advantage over from-scratch retrieval. Expert-service (EEM-backed) was tested across five models; agents-python (multi-step discovery, no EEM) serves as the baseline. Three rubrics — quality (reference-free A-grade), accuracy (0-1 vs expected), and Q/L/R/F (0-100 composite) — all show the same result from different angles.
...
| agents-python (no EEM) | 33% | 0.239 | 28.1 | ~350s |
...
The most controlled comparison in the three-way eval: Expert-service + Gemini 3.1 Pro scores 0.589 accuracy. agents-python, which also uses Gemini 3.1 Pro for synthesis, scores 0.239. Same synthesis model. 2.5x better accuracy. 10x faster. The only difference is the retrieval architecture — belief network vs multi-step document discovery.
...
agents-python produced 8 negative scores from penalty deductions for fabricated data or contradicting source documents (-47.0, -39.4, -27.0, -25.5, -17.25, -13.0, -10.5, -5.5). Expert-service models rarely go negative.
...
- agents-python (no EEM): 33% A-grade, 0.239 accuracy

Haiku (the smallest Claude model) with EEM scores 1.9x the A-grade and 2.6x the accuracy of agents-python without EEM. Gemma3 27B (an open model running locally) with EEM scores 2.4x the accuracy. This confirms the architectural ablation finding (2026-03-28): Sonnet+beliefs approximates Opus without beliefs. EEM compensates for model capability.
...
The AAP eval is the cleanest demonstration: EEM costs 0.53x and runs in 0.47x the time while being more accurate on both MC and OE. The three-way eval shows the same: expert-service queries complete in 9-35s vs ~350s for agents-python.
...
- Red Hat three-way: ~/git/redhat-expert/entries/2026/05/20/three-way-eval-comparison-expert-service-vs-agents-python-across-all-rubrics.md
